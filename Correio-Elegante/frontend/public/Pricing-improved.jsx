  ...
  const handleSubmit = useCallback(
    async (e) => {
      ...
      if (senderType === "identificado") {
        if (!senderName.trim()) return alert("Digite seu nome completo");
        if (senderCourse && !senderClassroom) return alert("Selecione a turma do remetente");
      }

      if (receiverType === "identificado" && !course) return alert("Selecione o curso");
      if (receiverType === "identificado" && course && !classroom) return alert("Selecione a turma");

      let finalProofUrl = null;

      if (paymentMethod === "pix") {
        if (!proofFile) return alert("Envie o comprovante");
        finalProofUrl = await uploadProof(proofFile);
        if (!finalProofUrl) return alert("Erro no upload do comprovante");
      }
        ...
        const { data, error } = await supabase.from("orders").insert([
          {
            ...
            sender_classroom: senderType === "anonimo" ? null : senderCourse ? senderClassroom : null,
